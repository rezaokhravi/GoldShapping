exports.process = function (command, onResult) {
    ...
    var onError = function (message) {
        console.log('cnn_report_message::', message);
    ...
    try {
        ...
        var onConnect = async function () {
            let queryString = '';
            await command.queryString.then(val => {
                queryString = val;
                if (command.queryString) query(queryString);
        ...
        var getConnectionStringConfig = function (connectionString) {
            ...
            console.log('getConnectionStringConfig::::', connectionString)
                    ...
                    if (match && match.length >= 2) {
                        ...
                        switch (match[0]) {
                            ...
                            case "multipleactiveresultsets":
                                config.options["multipleactiveresultsets"] = match[1];
                                break;
                            case "trusted_connection=":
                                config.options["trusted_connection"] = match[1];
                                break;
                            case "integrated security":
                                config.options["integrated security"] = match[1];
                                break;
                        }
                    }
                }
            }
            console.log('getConnection_config::::', config);
            return config;
        };

        const sql = require('mssql');
        const cnn = require('./db');
        const config = cnn.sqlConfig //getConnectionStringConfig(command.connectionString);
